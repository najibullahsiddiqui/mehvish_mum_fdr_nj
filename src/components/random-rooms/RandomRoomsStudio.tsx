...
import { Bot, Clapperboard, DoorOpen, Film, Rocket, Sparkles, Users, Video } from "lucide-react";
...
type StudioTab = "characters" | "rooms" | "episodes" | "production" | "render" | "automation";
...
const tabs: Array<{ key: StudioTab; label: string; shortLabel: string; icon: typeof Users }> = [
  ...
  { key: "automation", label: "Automation", shortLabel: "Automate", icon: Rocket },
];
    ...
    <section className="grid grid-cols-1 gap-5">
      <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
        <div className="grid gap-4 border-b border-stone-200 bg-gradient-to-r from-stone-950 via-stone-900 to-fuchsia-950 p-5 text-white lg:grid-cols-[1fr_auto] lg:items-center lg:p-6">
          <div>
            <div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-fuchsia-200"><Sparkles className="h-3.5 w-3.5" /> AI Entertainment Production System</div>
            <h2 className="flex items-center gap-2 text-2xl font-black tracking-tight"><Bot className="h-6 w-6 text-fuchsia-300" /> Random Rooms</h2>
            <p className="mt-1 text-sm font-medium text-stone-300">Plan once → generate voice and media → render → review the final vertical video.</p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center">
            {[["Characters", characterCount], ["Rooms", roomCount], ["Episodes", episodeCount]].map(([label, value]) => (
              <div key={String(label)} className="min-w-20 rounded-xl border border-white/10 bg-white/5 px-3 py-2"><div className="text-lg font-black">{value}</div><div className="text-[10px] font-black uppercase tracking-wide text-stone-400">{label}</div></div>
            ))}
          </div>
        </div>
